...
1. able to visualize data with very long axis with high resolution,
2. efficient to visualize periodic data.
...
where $b$ controls the distance between two loops. The distance between two neighbouring loops for a given $\theta$ is:

$$ d(\theta) = r(\theta + 2\pi) - r(\theta) = b \cdot (\theta + 2\pi) - b \cdot \theta = b \cdot 2\pi $$

This shows the distance between two neighbouring loops is independent to the value of $\theta$ and is a constant value. The following 
...
Since the distance between any two neighbouring loops for a given $\theta$ is
constant, it is a ideal place to put tracks along the curve where the tracks
...
the range of the spiral. Here the values should be in degrees and they are converted to radians internally.
...
end angle (in radians) of the spiral, the ranges of the viewport on both _x_-axis and _y_-axis that draw the spiral are $[-x, x]$ where
...
I suggest to set the values of `start` and `end` in a form of `360*a + b`, e.g. `360*4 + 90`, so that it is straighforward
...
# the right plot
spiral_initialize(start = 180, end = 360*5)
spiral_track()
```
...
Argument `scale_by` controls how to linearly interpolate the data on the
...
which are the same time points over different periods.
...
between two neighbouring loops in the spiral. In the following left plot, I added black border to the track 
...
Data range on _y_-axis is specified by the argument `ylim`. In the following example, I also added a _y_-axis
by `spiral_yaxis()`.
...
Also since dendrograms or phylogenetic trees do not have "periodic patterns",
...
the `phylo` object is converted to a `dendrogram` by the function `phylo_to_dendrogram()`, later `denddrogram::cutree.dendrogram()`
is applied to obtain the classifications.
...
cl = cutree(phylo_to_dendrogram(tree.hiv), k = 8)
tb = table(cl)
...
$(document).ready(function(){
    ...
    $("#TOC").on("sticky-start", function() {
        ...
        $("<p style='font-size:1.2em; padding-left:4px;'><a id='TOC-click'>Table of Content (click to open or close)</a></p>").insertBefore($("#TOC ul:first-child"));
        ...
        $("#TOC-click").click(function() {
            $("#TOC").children().first().next().toggle();
        });
